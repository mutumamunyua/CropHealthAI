...
import logging
from config import Config, geolocation_collection, extension_workers_collection
...
if extension_workers_collection is not None:
    extension_workers_collection.create_index([("location", "2dsphere")])

if geolocation_collection is not None:
    geolocation_collection.create_index([("location", "2dsphere")])

# Path to the CSV file
CSV_FILE_PATH = "extension_workers.csv"
...
# Function to insert extension workers data into MongoDB
def insert_extension_workers_data():
    try:
        with open(CSV_FILE_PATH, newline='', encoding='utf-8') as csvfile:
            ...
            for row in sampled_rows:
                ...
                if not first_name or not last_name or not services or not county or not town or not latitude or not longitude or not contact:
                ...
                if not closest_county or not closest_town:
                    logging.warning(f"⚠️ No close match found for {first_name} {last_name} in {town}, {county}. Skipping.")
                    continue

                # Check if geolocation already exists
                geolocation = geolocation_collection.find_one({"county": closest_county, "town": closest_town})
                if not geolocation:
                    logging.warning(f"⚠️ Geolocation not found for {first_name} {last_name} in {closest_town}, {closest_county}. Adding new geolocation data.")
                ...
                # Insert geolocation data in batches of 100
                ...
                # Insert extension workers data in batches of 100
                if len(data_to_insert) >= 100:
                    extension_workers_collection.insert_many(data_to_insert)
                    logging.info(f"✅ Inserted {len(data_to_insert)} new extension workers entries.")
            ...
            # Insert remaining extension workers data
            if data_to_insert:
                extension_workers_collection.insert_many(data_to_insert)
                logging.info(f"✅ Inserted {len(data_to_insert)} new extension workers entries.")

            if not data_to_insert:
                logging.info("⚠️ No extension workers data to insert.")
    except Exception as e:
        logging.error(f"⚠️ Error inserting extension workers data: {e}")

# Run the insertion
if __name__ == "__main__":
    insert_extension_workers_data()